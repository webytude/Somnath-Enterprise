...
<div class="toolbar bg-transparent pt-6 mb-5" id="kt_toolbar">
    <div id="kt_toolbar_container" class="container-fluid d-flex flex-stack">
        <div data-kt-swapper="true" data-kt-swapper-mode="prepend" data-kt-swapper-parent="{default: '#kt_content_container', 'lg': '#kt_toolbar_container'}" class="page-title d-flex flex-column align-items-start me-3 mb-5 mb-lg-0">
            <h1 class="d-flex text-dark fw-bolder fs-3 flex-column mb-0">Edit Tool</h1>
        </div>
    </div>
</div>
            ...
            <div class="col-xl-8">
                <div class="card card-flush h-lg-100" id="kt_tool_list_form_main">
                    <div class="card-body pt-5">
                        <form method="POST" id="kt_tool_list_form" class="form fv-plugins-bootstrap5 fv-plugins-framework" action="{{ route('tool-lists.update', $toolList->id) }}">
                            @csrf
                            @method('PUT')
                            ...
                            <div class="row mb-7">
                                <div class="col-md-6">
                                    ...
                                    <input type="number" class="form-control form-control-solid" name="quantity" value="{{ old('quantity', $toolList->quantity) }}" step="0.01" min="0" placeholder="Enter Quantity" />
                                    @error('quantity')
                                        <span id="error" class="error invalid-feedback" style="display: block;">{{ $message }}</span>
                                    @enderror
                                </div>
                                <div class="col-md-6">
                                    ...
                                    <input type="number" class="form-control form-control-solid" name="price" value="{{ old('price', $toolList->price) }}" step="0.01" min="0" placeholder="Enter Price" />
                                    @error('price')
                                        <span id="error" class="error invalid-feedback" style="display: block;">{{ $message }}</span>
                                    @enderror
                                </div>
                            ...
                            <div class="fv-row mb-7">
                                ...
                                <textarea class="form-control form-control-solid" name="remark" rows="3" placeholder="Enter Remark">{{ old('remark', $toolList->remark) }}</textarea>
                                @error('remark')
                                    <span id="error" class="error invalid-feedback" style="display: block;">{{ $message }}</span>
                                @enderror
                            </div>
