...
<div class="toolbar bg-transparent pt-6 mb-5" id="kt_toolbar">
    <div id="kt_toolbar_container" class="container-fluid d-flex flex-stack">
        <div data-kt-swapper="true" data-kt-swapper-mode="prepend" data-kt-swapper-parent="{default: '#kt_content_container', 'lg': '#kt_toolbar_container'}" class="page-title d-flex flex-column align-items-start me-3 mb-5 mb-lg-0">
            <h1 class="d-flex text-dark fw-bolder fs-3 flex-column mb-0">Add Daily Expense</h1>
        </div>
    </div>
</div>
            ...
            <div class="col-xl-6">
                <div class="card card-flush h-lg-100" id="kt_daily_expense_form_main">
                    <div class="card-body pt-5">
                        <form method="POST" id="kt_daily_expense_form" class="form fv-plugins-bootstrap5 fv-plugins-framework" action="{{ route('daily-expense.store') }}">
                            @csrf
                            ...
                            @else
                                <div class="fv-row mb-7">
                                    ...
                                    <select class="form-select form-select-solid" name="staff_id" id="staff_id" data-control="select2" data-placeholder="Select Staff...">
                                        ...
                                        @foreach($staff as $s)
                                            <option value="{{ $s->id }}" {{ old('staff_id') == $s->id ? 'selected' : '' }}>{{ $s->full_name }} ({{ $s->code ?? 'N/A' }})</option>
                                        @endforeach
                                    </select>
                                    @error('staff_id')
                            ...
                            <div class="fv-row mb-7">
                                ...
                                <input type="date" class="form-control form-control-solid" name="date" value="{{ old('date', date('Y-m-d')) }}" />
                                @error('date')
                                    <span id="error" class="error invalid-feedback" style="display: block;">{{ $message }}</span>
                                @enderror
                            </div>
                            <div class="fv-row mb-7">
                                ...
                                <input type="number" step="0.01" class="form-control form-control-solid" name="amount" value="{{ old('amount') }}" placeholder="Enter Amount" />
                                @error('amount')
                                    <span id="error" class="error invalid-feedback" style="display: block;">{{ $message }}</span>
                                @enderror
                            </div>
                            <div class="fv-row mb-7">
                                ...
                                <textarea class="form-control form-control-solid" name="description" rows="3" placeholder="Enter Description">{{ old('description') }}</textarea>
                                @error('description')
                                    <span id="error" class="error invalid-feedback" style="display: block;">{{ $message }}</span>
                                @enderror
                            </div>
