                    ...
                    <div class="card-body pt-5">
                        <form method="POST" id="kt_contractor_form" class="form fv-plugins-bootstrap5 fv-plugins-framework" action="{{ route('contractors.store') }}">
                            ...
                            <div class="fv-row mb-7">
                                ...
                                <input type="text" class="form-control form-control-solid" name="pedhi" value="{{ old('pedhi') }}" placeholder="Enter Pedhi" />
                                @error('pedhi')
                            ...
                            <div class="row mb-7">
                                <div class="col-md-4">
                                    ...
                                    <input type="text" class="form-control form-control-solid" name="payment_term" value="{{ old('payment_term') }}" placeholder="Enter Payment Term" />
                                    @error('payment_term')
                                        <span id="error" class="error invalid-feedback" style="display: block;">{{ $message }}</span>
                                    @enderror
                                </div>
                                <div class="col-md-4">
                                    ...
                                    <input type="number" class="form-control form-control-solid" name="amount" value="{{ old('amount') }}" step="0.01" min="0" placeholder="Enter Amount" />
                                    @error('amount')
                                        <span id="error" class="error invalid-feedback" style="display: block;">{{ $message }}</span>
                                    @enderror
                                </div>
                                <div class="col-md-4">
                                    ...
                                    <input type="number" class="form-control form-control-solid" name="remaining_amount" value="{{ old('remaining_amount') }}" step="0.01" min="0" placeholder="Enter Remaining Amount" />
                            ...
                            <div class="fv-row mb-7">
                                ...
                                <select class="form-select form-select-solid" name="payment_slab_id" id="payment_slab_id" data-control="select2" data-placeholder="Select Payment Slab...">
                                    ...
                                    @foreach($paymentSlabs as $slab)
                                        <option value="{{ $slab->id }}" {{ old('payment_slab_id') == $slab->id ? 'selected' : '' }}>{{ $slab->name }}</option>
                                    @endforeach
                                </select>
