            ...
            <div class="col-xl-6">
                <div class="card card-flush h-lg-100" id="kt_attendance_form_main">
                    <div class="card-body pt-5">
                        <form method="POST" id="kt_attendance_form" class="form fv-plugins-bootstrap5 fv-plugins-framework" action="{{ route('division.update', $division->id) }}">
                        @csrf
                        @method('PUT')
                            <div class="fv-row mb-7">
                                <label class="fs-6 fw-bold form-label mt-3">Division Name</label>
                                <input type="text" class="form-control form-control-solid" name="name" value="{{ $division->name }}" placeholder="Enter Division name" />
                                @error('name')
                            ...
                            <div class="fv-row mb-7">
                                ...
                                <select class="form-select form-select-solid" name="department_id" id="department_id" data-control="select2" data-placeholder="Select Department...">
                                    ...
                                    @foreach($departments as $dept)
                                        <option value="{{ $dept->id }}" {{ $dept->id == $division->department_id ? "selected" : "" }}>{{ $dept->name }}</option>
                                    @endforeach
                                </select>
                                @error('department_id')
                                    <span id="error" class="error invalid-feedback" style="display: block;">{{ $message }}</span>
                            ...
                            <div class="fv-row mb-7">
                                ...
                                <select class="form-select form-select-solid" name="subdepartment_id" id="subdepartment_id" data-control="select2" data-placeholder="Select Sub Department...">
                                    ...
                                    @foreach($subdepartments as $subdept)
                                        <option value="{{ $subdept->id }}" {{ $subdept->id == $division->subdepartment_id ? "selected" : "" }}>{{ $subdept->name }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="d-flex justify-content-end">
                                <a href="{{route('division.index')}}" data-kt-attendance-form="cancel" class="btn btn-light me-3">Cancel</a>
                                <button type="submit" data-kt-attendance-form="submit" class="btn btn-primary">
                                    <span class="indicator-label">Save Division</span>
...
<script>
    $(document).ready(function() {
        $('#department_id').on('change', function() {
            ...
            var currentSubdepartmentId = {{ $division->subdepartment_id }};
