@section('title','Edit Bill Inward')
@extends('admin.layouts.main')
@section('main_contant')
<div class="toolbar bg-transparent pt-6 mb-5" id="kt_toolbar">
    <div id="kt_toolbar_container" class="container-fluid d-flex flex-stack">
        <div data-kt-swapper="true" data-kt-swapper-mode="prepend" data-kt-swapper-parent="{default: '#kt_content_container', 'lg': '#kt_toolbar_container'}" class="page-title d-flex flex-column align-items-start me-3 mb-5 mb-lg-0">
            <h1 class="d-flex text-dark fw-bolder fs-3 flex-column mb-0">Edit Bill Inward</h1>
        </div>
    </div>
</div>
            ...
            <div class="col-xl-12">
                <div class="card card-flush h-lg-100" id="kt_bill_inward_form_main">
                    ...
                    <div class="card-body pt-3">
                        <form method="POST" id="kt_bill_inward_form" class="form fv-plugins-bootstrap5 fv-plugins-framework" action="{{ route('bill-inwards.update', $billInward->id) }}" enctype="multipart/form-data">
                            @csrf
                            @method('PUT')
                                    ...
                                    <div class="col-12">
                                        ...
                                        <select class="form-select form-select-solid" name="firm_id" id="firm_id" data-control="select2" data-placeholder="Select Firm..." required>
                                            ...
                                            @foreach($firms as $firm)
                                                <option value="{{ $firm->id }}" {{ old('firm_id', $billInward->firm_id) == $firm->id ? 'selected' : '' }}>{{ $firm->name }}</option>
                                            @endforeach
                                        </select>
                                        @error('firm_id')
                            ...
                            <div class="mb-10">
                                ...
                                <div class="row g-5 mb-7">
                                    <div class="col-12 col-lg-4">
                                        ...
                                        <select class="form-select form-select-solid" name="party_id" id="party_id" data-control="select2" data-placeholder="Select Party..." required>
                                            ...
                                            @foreach($parties as $party)
                                                <option value="{{ $party->id }}" data-gst="{{ $party->gst }}" data-pan="{{ $party->pancard }}" {{ old('party_id', $billInward->party_id) == $party->id ? 'selected' : '' }}>{{ $party->name }}</option>
                                            @endforeach
                                        </select>
                                        @error('party_id')
                                            <span id="error" class="error invalid-feedback" style="display: block;">{{ $message }}</span>
                                    ...
                                    <div class="col-12 col-lg-4">
                                        ...
                                        <input type="text" class="form-control form-control-solid" id="party_gst" name="party_gst" value="{{ old('party_gst', $billInward->party_gst) }}" placeholder="Auto-filled from Party" readonly />
                                    </div>
                                    <div class="col-12 col-lg-4">
                                        <label class="fs-6 fw-bold form-label mt-3">Party Pan (Auto)</label>
                                        <input type="text" class="form-control form-control-solid" id="party_pan" name="party_pan" value="{{ old('party_pan', $billInward->party_pan) }}" placeholder="Auto-filled from Party" readonly />
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="mb-10">
                                ...
                                <div class="row g-5 mb-7">
                                    <div class="col-12 col-lg-4">
                                        ...
                                        <input type="text" class="form-control form-control-solid" name="bill_number" value="{{ old('bill_number', $billInward->bill_number) }}" placeholder="Enter Bill Number" />
                                        @error('bill_number')
                                            <span id="error" class="error invalid-feedback" style="display: block;">{{ $message }}</span>
                                        @enderror
                                    </div>
                                    <div class="col-12 col-lg-4">
                                        ...
                                        <input type="date" class="form-control form-control-solid" name="bill_date" value="{{ old('bill_date', $billInward->bill_date ? $billInward->bill_date->format('Y-m-d') : '') }}" />
                                        @error('bill_date')
                                            <span id="error" class="error invalid-feedback" style="display: block;">{{ $message }}</span>
                                        @enderror
                                    </div>
                                    <div class="col-12 col-lg-4">
                                        ...
                                        @if($billInward->bill_attachment)
                                            <div class="mb-2">
                                                <a href="{{ $billInward->bill_attachment }}" target="_blank" class="btn btn-sm btn-light-primary">
                                    ...
                                    <table class="table align-middle table-row-dashed fs-6 gy-5" id="material-details-table" style="min-width: 1200px;">
                                        ...
                                        <tbody id="material-details-tbody">
                                            @php
                                                ...
                                                $existingDetails = $billInward->details;
                                ...
                                <div class="col-12 col-md-8 col-lg-5">
                                    <div class="bg-light-primary rounded p-5">
                                        <div class="mb-5">
                                            ...
                                            <input type="number" class="form-control form-control-solid" name="add_bhadu_labour" id="add_bhadu_labour" step="0.01" min="0" value="{{ old('add_bhadu_labour', $billInward->add_bhadu_labour) }}" placeholder="0.00" />
                                        </div>
                                        <div>
                                            <label class="fs-6 fw-bold form-label">Total Bill/Voucher Amt.</label>
                                            <input type="number" class="form-control form-control-solid" name="total_bill_amount" id="total_bill_amount" step="0.01" min="0" value="{{ old('total_bill_amount', $billInward->total_bill_amount) }}" placeholder="A+B" readonly />
                                        </div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="mb-7">
                                ...
                                <textarea class="form-control form-control-solid" name="remarks" rows="3" placeholder="Enter Remarks">{{ old('remarks', $billInward->remarks) }}</textarea>
                                @error('remarks')
                                    <span id="error" class="error invalid-feedback" style="display: block;">{{ $message }}</span>
                                @enderror
                            </div>
                            ...
                            <div class="mb-10">
                                ...
                                <div class="row g-5 mb-7">
                                    <div class="col-12 col-lg-4">
                                        ...
                                        <select class="form-select form-select-solid" name="payment_status" id="payment_status" required>
                                            <option value="Pending" {{ old('payment_status', $billInward->payment_status) == 'Pending' ? 'selected' : '' }}>Pending</option>
                                            <option value="Paid" {{ old('payment_status', $billInward->payment_status) == 'Paid' ? 'selected' : '' }}>Paid</option>
                                        </select>
                                ...
                                <div class="row g-5 mb-7" id="payment-details-section" style="display: {{ old('payment_status', $billInward->payment_status) == 'Paid' ? 'block' : 'none' }};">
                                    <div class="col-12 col-lg-4">
                                        <label class="fs-6 fw-bold form-label mt-3">If Paid - Ref. No.</label>
                                        <input type="text" class="form-control form-control-solid" name="payment_ref_number" id="payment_ref_number" value="{{ old('payment_ref_number', $billInward->payment_ref_number) }}" placeholder="Enter Reference Number" />
                                    ...
                                    <div class="col-12 col-lg-4">
                                        ...
                                        <input type="date" class="form-control form-control-solid" name="payment_date" id="payment_date" value="{{ old('payment_date', $billInward->payment_date ? $billInward->payment_date->format('Y-m-d') : '') }}" />
                                        @error('payment_date')
                                            <span id="error" class="error invalid-feedback" style="display: block;">{{ $message }}</span>
                                        @enderror
                                    </div>
                                    <div class="col-12 col-lg-4">
                                        ...
                                        <textarea class="form-control form-control-solid" name="payment_remarks" id="payment_remarks" rows="2" placeholder="Enter Payment Remarks">{{ old('payment_remarks', $billInward->payment_remarks) }}</textarea>
...
<script>
    $(document).ready(function() {
        ...
        var currentPartyId = {{ $billInward->party_id ?? 'null' }};
